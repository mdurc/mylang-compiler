...
/* this is mostly ported/translated from the x86_64 backend which was written first */
...
private:
  ...
  void load_into_reg(const IROperand& operand, const std::string& dst_reg, std::uint64_t size);
  std::string load_to_reg(const IROperand& operand, std::uint64_t size);
  void store_to_operand(const IROperand& dst, const std::string& src_reg, std::uint64_t size);
  std::string emit_get_var_addr(const IR_Variable& ir_var);

  std::string get_sized_reg(const std::string& reg_name, std::uint64_t size);
  std::string get_ldr_instr(const std::string& reg, const std::string& addr_reg, std::uint64_t size);
  std::string get_str_instr(const std::string& reg, const std::string& addr_reg, std::uint64_t size);
  ...
  void handle_prim_binop(const std::string& aarch64_instr, const IRInstruction& instr);
  ...
  void handle_shift(const std::string& aarch64_instr, const IRInstruction& instr);

  void handle_cmp(const IRInstruction& instr);
